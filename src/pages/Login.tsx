...
// 4. Update Login.tsx to improve the flow
// Modify your existing Login.tsx
...
export const Login: React.FC = () => {
  ...
  useEffect(() => {
    // If user is already authenticated, redirect to portfolio
    if (isAuthenticated) {
      navigate('/portfolio');
    }
  }, [isAuthenticated, navigate]);
  ...
  const handleSignIn = async () => {
    try {
      await signIn();
      // The redirect will happen in the auth flow
    } catch (error) {
